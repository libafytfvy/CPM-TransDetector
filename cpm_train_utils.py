...
def accuracy(y_hat, y):
    ...
    # y_hat将大于0.5的置为1，将小于0.5的置为0
    y_hat = torch.where(y_hat > 0.5, torch.ones_like(y_hat), torch.zeros_like(y_hat))
    # y_hat=y为true，y_hat！=y为false
    cmp = d2l.astype(y_hat, y.dtype) == y
    # d2l.astype(cmp, y.dtype)将cmp中的true置为1，false置为0
    # d2l.reduce_sum(d2l.astype(cmp, y.dtype))矩阵相加，计算正确的总数
...
def evaluate_accuracy(net, data_iter, loss):
    ...
    with torch.no_grad():
        for X, y in data_iter:
            l = loss(net(X)[0], y)
            metric.add(float(l), accuracy(net(X)[0], y), d2l.size(y))
    return metric[0], float((metric[2] - metric[1]) / metric[2])
...
def train_epoch_ch3(net, train_iter, loss, updater):
    ...
    for X, y in train_iter:
        ...
        # if b is None or use_random_iter:
        #     # 在第⼀次迭代或使⽤随机抽样时初始化state
        #     state = init_rnn_state(batch_size=X.shape[0], num_hiddens=256, device=device)
        # else:
...
# Defined in file: ./chapter_linear-networks/softmax-regression-scratch.md
def train_ch3(net, train_iter, test_iter, loss, num_epochs, updater,path):
    ...
    for epoch in range(num_epochs):
        ...
        print(epoch + 1, "train_loss:", train_metrics[0], " train_bit_err:", train_metrics[1], " val_loss:",
              test_metrics[0], " val_bit_err:", test_metrics[1])
        if best_performance is None or test_metrics[0] < best_performance:
            best_performance = test_metrics[0]
            torch.save(net.state_dict(), "Transformer_pth/train_new422_M4_3RC_h0.25_6dB_Transformer.pth")
